...
- [ ] 01-01-PLAN.md — Package scaffold (pyproject, src layout), storage enums and transition tables, all ten SQLModel tables, Alembic initial migration, pytest harness (wave 1)

**Wave 2** *(blocked on Wave 1 completion)*

- [ ] 01-02-PLAN.md — Process config, loopback bind guard, CSRF middleware, app factory with migrations in lifespan, Butter-token shell with MOCK and AUTOMATION OFF badges, vendored htmx, settings screen persisted in SQLite (wave 2)
- [ ] 01-03-PLAN.md — SecretStore seam (keyring or 0600 file, loud fallback) and log redaction with token-leak test (wave 2)
- [ ] 01-04-PLAN.md — Connector protocol, DTOs and error classes, fixture-backed MockConnector with scenario queue (pagination, near-limit, 429, auth failure, timeout, permanent error), connector factory (wave 2)
- [ ] 01-05-PLAN.md — LLM client protocol, deterministic FakeLLMClient, OllamaClient with schema-constrained output and image input, respx tests (wave 2)

**Wave 3** *(blocked on Wave 2 completion)*

- [ ] 01-06-PLAN.md — InstagramGraphConnector (Instagram Login scopes, cursor pagination, usage headers, token refresh, error classes, no retry on POST, no delete) with respx contract tests (wave 3)
- [ ] 01-07-PLAN.md — Wire secret store, connector and LLM factories into the app, connection status screen with AUTH PROBLEM block, README with install/run/test commands, end-to-end smoke test (wave 3)
...
|-------|----------------|--------|-----------|
| 1. Foundation and Connectors | 0/7 | Not started | - |
