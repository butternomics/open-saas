...
**Domain:** Local-first Instagram comment-reply assistant (Python app, local browser UI, SQLite, Ollama local LLM + vision, whisper.cpp, Instagram Graph API via Instagram Login)
**Researched:** 2026-09-25
**Confidence:** HIGH for library versions and API mechanics (verified via PyPI JSON endpoints and official Meta docs); MEDIUM for Ollama structured-output edge cases and whisper.cpp binding choice (fast-moving, thinner docs); LOW flagged inline where only WebSearch summaries were available.
...
```bash
# Core
pip install fastapi jinja2 python-multipart "uvicorn[standard]"
...
Serve htmx as a vendored static file (`static/htmx.min.js`) rather than a package — no JS package manager needed.

System-level (not pip-installable, must exist on the deployment machine, not the build container):
...
**If the deployment machine has no GPU (CPU-only inference):**
...
**If the deployment machine runs headless (no logged-in desktop session) Linux:**
...
**If pywhispercpp fails to build/install on the target hardware:**
...
- **Key endpoints:**
  - `GET /{ig-media-id}/comments` — list comments on a piece of media.
  - `GET /{ig-comment-id}/replies` — list replies on a comment (for reconciliation / "already handled" checks).
  - `POST /{ig-comment-id}/replies` — publish a reply to a comment (the actual send action).
  - `POST /{ig-comment-id}` with a hide param — hide/unhide a comment (not required by the brief's scope, but useful to know exists).
  - `DELETE /{ig-comment-id}` — delete a comment (explicitly out of scope per the brief: "no automatic deletion of audience comments" — do not implement this call path at all).
  HIGH confidence (Meta's own IG Comment / Replies reference docs).
...
---
*Stack research for: local-first Instagram comment-reply assistant*
*Researched: 2026-09-25*
